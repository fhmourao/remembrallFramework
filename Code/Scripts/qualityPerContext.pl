...
while( defined($line = <TEST_FILE>) ){
	...
	foreach $item (@items){
		...
		if( exists($hashOfTrainItems{$userId}{$itemId}) ){
			...
			if( exists($numItemsPerScore{$score}) ){
				$numItemsPerScore{$score} += 1;
			}
			else{
				$numItemsPerScore{$score} = 1;
			}
	...
	foreach $recommendation (@recommendations){
		...
		if(exists($testItems{$itemId})){
			if( exists($numHitsPerScore{$score} ) ){
				$numHitsPerScore{$score} += 1;
			}
			else{
				$numHitsPerScore{$score} = 1;
			}
...
foreach $score ( sort {$a <=> $b} keys(%numItemsPerScore)){
	if( exists($numHitsPerScore{$score}) ){
		print FOUT $score, "\t",$numHitsPerScore{$score}/$numItemsPerScore{$score}, "\t", $numItemsPerScore{$score}, "\n";
	}
}
close(FOUT);
...
sub loadTrainSet{
	...
	while( defined($line = <TRAIN_FILE>) ){
		...
		$trainingSizePerUser{$userId} = $numTimeUnits;
		for($i=0; $i<$numTimeUnits; $i++){
			...
			foreach $item (@items){
				...
				$hashOfTrainItems{$userId}{$itemId} = 1;
			}
			splice(@items);

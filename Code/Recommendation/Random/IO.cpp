...
short getArguments(char **trainFile, char **testFile, char **outputFile, float &percentageOfHistory, int argc, char **argv){
	int opt; 
	...
	*trainFile = NULL;
	...
	while( (opt = getopt(argc, argv, "ho:t:e:r:")) != -1 ) {
		switch(opt) {
			case 'h':
				printUsage();
				return 0;
	...
	if( *outputFile == NULL || *testFile == NULL || *trainFile == NULL || percentageOfHistory < 0.0){
		printUsage();
		return 0;
	}

/************************************************************************
                        sampleTrainingItems.cpp
************************************************************************/

#include "sampleTrainingItems.h"
...
int main(int argc, char **argv){
	...
	if( !(getArguments(&trainFileName, &testFileName, &outputFileName, percentageOfHistory, argc, argv)) ){
		return 1;
	}
	...
	while( !testFile.eof() ){
		...
		// get random sample from user history
		recommendForgottenItems(forgottenItems, currentUser);

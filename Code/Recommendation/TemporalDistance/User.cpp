...
std::set<int> User::getCandidateItems(){
	return this->candidateItems;
}

void User::loadCandidateSet(int trainingUser, std::ifstream &inputFile){
	...
	if(!inputFile.eof()) {
		...
		this->testMoment = atoi(vetor[1].c_str());
		...
		for(int i=2; i<vectorSize; i++){
			...
			this->candidateItems.insert( atoi(itemId.c_str()) );
		}		
	}	

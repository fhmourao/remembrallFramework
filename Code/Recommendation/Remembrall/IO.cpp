...
short getArguments(char **trainFile, char **testFile, char **candidateFile, char **outputFile, int &contextSize, float &recommendationPercentage, int argc, char **argv){
	...
	*trainFile = NULL;
	...
	while( (opt = getopt(argc, argv, "ho:t:e:a:c:r:")) != -1 ) {
		switch(opt) {
			case 'h':
				printUsage();
				return 0;
	...
	if( *outputFile == NULL || *testFile == NULL || *candidateFile == NULL || *trainFile == NULL 
		|| contextSize < 0 || recommendationPercentage < 0 || recommendationPercentage > 1.0 ){
		printUsage();
		return 0;
	}
...
void printRecommendations(User currentUser, long int testMoment, vector<myPair> &forgottenItems, int recommendationSize, std::ofstream &outputFile){
	...
	for(int i=0;  (i < numItems) && (i < recommendationSize); i++) {
